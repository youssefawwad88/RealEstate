...
def score_deal_viability(
    residual_land_value: float, asking_price: float, land_pct_gdv: float, breakeven_pct_market: float
) -> Dict[str, Any]:
    """
...
def get_color_indicator(score: Literal["green", "yellow", "red"]) -> str:
    ...
    Returns:
        Emoji string
    """
    color_map = {"green": "✅", "yellow": "⚠️", "red": "❌"}
    return color_map.get(score, "❓")


def calculate_sensitivity(base_value: float, sales_down_10pct: float = None, costs_up_10pct: float = None) -> Dict[str, float]:
    """

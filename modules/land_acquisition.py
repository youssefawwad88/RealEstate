...
def validate_inputs(inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    required_fields = [
        'land_area_sqm', 'asking_price', 'far', 'coverage',
        'efficiency_ratio', 'expected_sale_price_psm', 
        'construction_cost_psm', 'profit_target_pct'
    ]
...
def load_historical_data(file_path: Optional[str] = None) -> pd.DataFrame:
    ...
    try:
        ...
        columns = [
            'site_name', 'land_area_sqm', 'asking_price', 'zoning',
            'far', 'coverage', 'max_floors', 'efficiency_ratio',
            'expected_sale_price_psm', 'construction_cost_psm',
            'soft_cost_pct', 'profit_target_pct', 'taxes_fees'
        ]
